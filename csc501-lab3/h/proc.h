...
#define getpri(x)	(x->ipp!=0?x->ipp:x->pprio)
...
struct	pentry	{
	...
	int	pwaitret;
	int 	ipp;			/* inherited process priority 	*/
	int 	lock_id;			/* lock for which process waiting	*/
	int		plbefore[50];		/* locks used in the past: if set, shouldn't be reset during the process' execution	*/
	int		plafter[50];		/* locks acquired by the process	*/
};
